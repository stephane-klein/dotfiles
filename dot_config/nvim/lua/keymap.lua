...
-- Generated by ChatGPT4
    function _G.try_tag_then_help()
        -- Gets the word under the cursor
        local word = vim.fn.expand("<cword>")
        -- Retrieves the list of tags for the word
        local tags = vim.fn.taglist('^' .. word .. '$')

        -- Checks if any tags were found
        if #tags > 0 then
            -- If tags exist, uses :tag to go to the first found tag
            vim.cmd('tag ' .. word)
        else
            -- Tries to use :help on the word, and catch if it fails
            local success, _ = pcall(vim.cmd, 'help ' .. word)
            if not success then
                -- If :help command fails, display an error message
